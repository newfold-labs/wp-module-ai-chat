...
export const isInitialGreeting = (content) => {
	...
	const hasGreetingKeywords = (
		...
		(normalized.includes('today') || normalized.includes('feel free') || normalized.includes('ask'))
	);
	
	// Check for very short messages that are likely greetings
	const isShortGreeting = normalized.length < 150 && (
		(normalized.includes('hello') && (normalized.includes('assist') || normalized.includes('help'))) ||
